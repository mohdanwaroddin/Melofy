...
//we will get playlist ID as a route parameter and return the playlist having that ID
...
router.post(
    ...
    async (req, res) => {
        ...
        //step 2: check if song is a valid song
        const song = await Song.findOne({ _id: songId });
        if (!song) {
            return res.status(304).json({ err: "Song does not exist" });
        }

        //step3: now we can add song  into playlist
